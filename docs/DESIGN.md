...
- *2026-06-29* — **T7.27 landed — diagnostic tooltip readability.** The hover tooltip was unreadable: CodeMirror's default `.cm-tooltip` is a fixed light-grey chip that ignores our CSS-var theme, so in the dark theme it rendered light `--fg` text on a light chip (invisible until selected); the rows also reused the squiggle class (`cm-diag-error`), wavy-underlining the message text. Fix (`diagnostics.ts`): a new `EditorView.theme` (outweighs CM's baseTheme default) re-skins `.cm-tooltip.cm-tooltip-hover` to the semantic tokens — `--bg`/`--fg`/`--border`, rounded + soft shadow, mirroring the completion popup's chrome. `diagnosticTooltipDom` rebuilt: each row carries a **severity-coloured left accent** (`--error`/`--warning`/`--info`), the **message in `--fg`**, and **help on a dimmed `--muted` line** below — distinct classes from the squiggle marks, so tooltip text is never itself underlined; multi-diagnostic tooltips get a divider. Added an `--info` semantic token (light/dark/forced blocks in `app.css`) so the info severity has a real token like `--error`/`--warning` (used here and by the T7.28 panel). Squiggle wavy underlines left as-is (already correct, including the `-webkit-` prefix). Tests: `diagnostics.test.ts` updated to assert the new message/help DOM + severity-keyed row class. Background/border `var(...)` need no WKWebView prefixing. Gate green; visual eyeball deferred (hover state is hard to screenshot) — spacing/shadow values provisional, matched to the completion popup.
- *2026-06-29* — **T7.28 landed — diagnostics panel + bottom button.** The bottom-bar problem indicator became a `<button>` (`aria-haspopup="listbox"`, `aria-expanded`) that toggles an exhaustive problems popover; clicking an entry jumps the active editor's selection to its span (scrolled into view). New pure `diagnosticEntries(source, diagnostics)` (`diagnostics.ts`): each diagnostic gets a 1-based `line:col`, **sorted by source position** so the list reads top-to-bottom like the squiggles, and it **matches the bottom-bar count** (keeps out-of-range stale spans, flagged `inRange:false` rather than dropped). New `DiagnosticsPanel.svelte` — popover list: severity Material Symbol keyed to `--error`/`--warning`/`--info`, message, dimmed help, right-aligned `line:col`; unreachable entries disabled (show `—`, guarded so a programmatic click is a no-op too). `BottomBar.svelte` anchors the popover above the bar (right-aligned to the viewport), dismisses on Escape / outside-pointer (mirrors the export menu), and auto-closes when the doc goes clean or a notice flashes; the clean "No problems" state stays a quiet non-interactive indicator. `App.svelte` passes the active `source` + `onJumpToDiagnostic`, which **reuses the render→source jump path** (`handlePrimitiveClick`) on the active doc. **Decisions:** scoped to the **active document** (the bottom-bar count already was, so they stay consistent — a cross-document list would need per-doc grouping + active-doc switching); the jump sets selection + scrolls but **doesn't grab editor focus** (parity with the render-primitive click). Tests: `diagnosticEntries` (location/sort/stale), `DiagnosticsPanel.test.ts` (render/sort/select/disabled-stale), `BottomBar.test.ts` (open→jump→close, Escape, no-button-when-clean). Full gate + coverage green (357 JS tests; TS-only, no core/wasm change).
- *2026-06-29* — **T7.29 landed — help view.** A getting-started reference, opened as a tab from a new bottom-bar **help** button. Registered as a **global singleton** in the view registry (`workspace.ts`) but — unlike the bottom bar (chrome) — **placed in a group as a tab**: one shared instance, no document (`viewInstance("help")` → id `"help"`, idempotent `addTab`). The tab strip already falls back to the registry title/icon for doc-less tabs (`tabLabel`), so no shell change was needed; `App.svelte` renders it in the workspace snippet and `openHelp()` opens-or-focuses it in the editor group. New `HelpView.svelte` — a scrollable, themed reading column (semantic tokens: accent inline code, muted help text, bordered `<kbd>` keys): welcome intro, **the workspace** (tabs/groups/dock/bottom-bar), **writing a score** (annotated header+`score{}` sample; notes `string:fret` + `.t/.i/.m` fingers + `_N` durations; pinches `[ ]`; ties `~`; rests; techniques-as-functions; structure `section`/`chord`/`repeat`+`ending`/`pickup`/`loop`/`measure`; reuse `def`/`import`/def-gallery), and a **keyboard-shortcuts** table. `BottomBar.svelte` gained the help button (bottom-left by the dock toggle) + `onOpenHelp`. **Decisions:** help button placed bottom-left (keeps the right cluster as settings/status); content scoped to a one-screen reference since it overlaps M8's T8.3 (deeper docs grow there). Zoom keys while the help tab is focused fall through to the render target (same as preview focus) — harmless. Tests: registry kind/keys (`workspace.test.ts`), help button (`BottomBar.test.ts`), App open→tab→idempotent-reopen. Full gate + coverage green (359 JS tests, HelpView 100% lines; TS-only, no core/wasm change).
- *2026-06-29* — **T7.30 landed — native desktop menu bar (D48).** The Tauri app gets a native menu built in **JS** (`@tauri-apps/api/menu`) so every item dispatches the **same App handler** the in-app controls call — one command source, no drift. `menu.ts` splits a pure `appMenuModel({mac, templates})` (plain `MenuNode[]` data, unit-tested) from a thin `installAppMenu(handlers, opts)` glue that translates the model into Tauri `Menu`/`Submenu`/`MenuItem`/`PredefinedMenuItem` and `setAsAppMenu()`s it (desktop-only; wrapped in try/catch so a missing runtime — e.g. under jsdom tests — silently no-ops). **Layout:** *cadtab* app menu (macOS only: About/Services/Hide/Quit) · **File** (New ▸ templates, Open… `⌘O`, Open Folder… `⌘⇧O`, Save `⌘S`, Export ▸ SVG/PNG/PDF/Bundle, Close Tab `⌘W`) · **Edit** (native Cut/Copy/Paste/Select All) · **View** (Zoom In/Out/Reset, Toggle Project Dock `⌘B`) · **Help**. `App.svelte` wires a `menuHandlers` map to the existing functions and installs once on desktop; the three App-level keyboard handlers (`onIOKey`/`onDockKey`/`onCloseTabKey`) became **web-only** so the native menu's accelerators own those shortcuts on desktop — one dispatch path per platform, no double-fire. **Decisions:** (1) **zoom stays click-only in the menu** — its `=`/`+` duality can't map to a single accelerator, so `onZoomKey` keeps handling zoom on both platforms; (2) **Undo/Redo omitted from Edit** — predefined items carry their own accelerators, and a native Undo `⌘Z` would shadow CodeMirror's own history before it sees the key; CodeMirror keeps `⌘Z`/`⌘⇧Z` (a menu-driven undo routed into the focused editor is a clean follow-up); (3) **New is a template submenu** (a native item can't pop the in-app chooser); (4) macOS detected via `navigator.userAgent` (no new dep); items always-enabled (commands no-op safely with no active doc). Tests: `menu.test.ts` (model structure, accelerator placement, exhaustive command coverage, mac-vs-non-mac leading menu); the glue is the untested desktop-only seam (menu.ts 98% lines). Full gate + coverage green (365 JS tests; TS-only, no core/wasm/Rust change). **Acceptance:** native dispatch/accelerator-consumption and `⌘W`-doesn't-close-window need a real `just dev` desktop run — the JS model is verified but the OS behavior can't be exercised in jsdom.
- *2026-06-29* — **T7.31 landed — syntax-highlighting palette.** The token colours were hardcoded One Light hexes (bright purple/orange/green/blue) that never adapted to the theme — so on the dark-by-default ground they painted light-mode values. Replaced them with four semantic palette tokens in `app.css` (`--syn-structure`/`--syn-number`/`--syn-string`/`--syn-comment`), defined in every theme block (light default + `prefers-color-scheme: dark` + forced light + forced dark) so highlighting re-themes with the rest of the UI like the other tokens. **Muted two-tone scheme:** keywords **and operators** share one desaturated-blue *structure* tone (`#3f6e96` light / `#84a8c9` dark); numbers are warm tan (`#97652b`/`#c9a578`); strings muted green (`#4e7a3e`/`#8faa74`); comments a gray italic (`#8a8a8a`/`#767676`). Idents and punctuation stay `--fg` so fretted positions (`3:0`) and braces read as plain ink. `highlight.ts`'s `tokenTheme` now references the `var(--syn-*)` tokens (baseTheme keeps lowest precedence; `var()` resolves up to `:root`). **Active line:** the full-width `.cm-activeLine` fill (`Editor.svelte`) gave way to a faint left-edge tick — `boxShadow: inset 2px 0 0 color-mix(--accent 55%)`, background transparent — so the row is marked without washing out the caret/selection/token colours on it; the gutter active-line fill is left as-is. Tests unaffected (they assert token *classes*, not colours). Gate green (365 JS tests; CSS/TS-only, no core/wasm/Rust change). **Provisional (refine by feel):** exact hues, the tick's 55% accent, and whether the gutter fill should also become a tick. **Inspiration:** the user shared a Zed reference (`docs/zed-inspiration.png`) confirming the low-saturation, harmonious direction; its small-detail polish (file-type icons, git-status filename colour, full-row dock highlight, brightening active line number, breadcrumb-with-symbol, accent restraint, thin dividers) is the bar for the rest of the Cohesion group — esp. T7.34/T7.34a. The PNG was colour-sampled (ImageMagick) for concrete targets: a chrome→dock→editor **elevation stack** that *darkens* toward the editing surface (`#313337`→`#1F2126`→`#0E1015`; ours is flat `#1e1e1e`), warm-dominant syntax (orange keyword / tan function / sky-blue type / lavender number / gold git-modified accent), and a hard active-line-number brighten — all banked for the T7.34 retune.
- *2026-06-29* — **T7.32 landed — bidirectional highlight treatment.** The cursor↔primitive highlight reused the warm `--accent`, recolouring the engraved fret number orange — it read as if the *notation* changed, not as a selection. Replaced with a calm **highlighter-fill** treatment (chosen over solid-recolour and a paint-order halo after side-by-side mockups — the halo was too subtle): the active glyph **keeps its ink** and sits on a soft rounded **`--select` chip**, like highlighted text. New dedicated semantic token **`--select`** in `app.css` (light/dark/forced blocks), decoupled from the warm `--accent` and seeded from the T7.31 structure-blue family (`#3f6e96` light / `#84a8c9` dark), so the highlight is tunable on its own. `Tab.svelte`: the painter carries no font metrics, so a `textChip(prim)` helper **estimates the chip box** from the glyph count × `style.size` plus generous padding (`+0.6` wide / `+0.48` tall, `rx 0.36`), anchoring centred vs. start-anchored roles correctly; the `<rect class="active-chip">` is drawn **before** the glyph (so it sits under the ink) and is `pointer-events: none` (the number stays the click target). Fill is `color-mix(--select 42%, transparent)`. Open-curve paths (ties/slides/bends) have no interior to fill, so the active path takes the `--select` stroke (thickened to `0.1`). The old `--tab-accent` token + `text.active`/`path.active` recolour rules are gone. **This chip treatment is what T7.33 reuses** to light repeat barlines / volta brackets / `measure {}` boxes. Tests: existing Tab `.active`-class assertions still pass (class still applied; colours untested). Gate green (365 JS tests; CSS/TS-only, no core/wasm/Rust change). **Provisional (refine by feel):** chip opacity (42%), padding/roundness, and hue (structure-blue vs. the brighter Zed type-blue). Eyeballed via rsvg mockups on both themes; the live WKWebView render is a clean follow-up check.
